...
A lack of thrown errors does not mean your code dom functions is completely correct.

An incredibly powerful tool for locating bugs is a method you likely learned very early on in your JavaScript journey:
 console.log()!
...
/*Exercise without correction

In the code editor, we’ve given you a function that is supposed to take a string containing a single word
as an argument and return the capitalized version of that word. If the string contains multiple words (contains a space character, ' '),
 it should return null.

Currently, this code is not working as expected.
Take a look at our tests on lines 16-19. We expect the first function call to return 'Hey' and the second function call to return null.
Run the code to see that it is not working as expected but also not throwing any errors.
...
// exercise corrected underneath
function capitalizeASingleWord(word) {
// check to see if there is a space in the words if yes should return null other wise proceed
    // if one word is passed it is capitalized
    if (word.match(' ')) {
        console.log('Word value inside of if statement: ' + word);
        return null;
    }
